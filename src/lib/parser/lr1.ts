...
function closure_lr1(items: LR1Item[], g: Grammar, first: Map<string, Set<string>>): LR1Item[] {
	const result = [...items];
...
export function compute_lr1(
	...
	const start_alt =
		g.mode === 'compact' ? start_prod.children[0].split('') : start_prod.children[0].split(' ');
	...
	const state_key = (items: LR1Item[]) => items.map(item_key_lr1).sort().join('|');
	...
	while (i < states.length) {
		...
		const syms = new Set(state.items.map((item) => item.alt[item.dot]).filter(Boolean) as string[]);
	...
	const set = (key: string, val: LRAction) => {
		if (action.has(key)) {
			...
			if (existing.type === val.type) {
				if (existing.type === 'shift' && val.type === 'shift' && existing.state === val.state)
					return;
				if (
					existing.type === 'reduce' &&
					val.type === 'reduce' &&
					existing.prod.parent === val.prod.parent &&
					existing.alt.join('') === val.alt.join('')
				)
					return;
			}
