...
export function astToDot(node: ASTNode, parentId?: string): { dot: string; id: number } {
	...
	function traverse(n: ASTNode, parent?: string): string {
		...
		if (n.symbol === '' || n.symbol === 'ε') {
			// Epsilon node
			nodes.push(
				`  ${currentId} [label="ε", shape=plaintext, fontsize=11, fontcolor="#999999", fontname="Fira Mono"];`
			);
		} else if (isTerminal(n)) {
			// Terminal node — green filled ellipse
			nodes.push(
				`  ${currentId} [label="${label}", shape=ellipse, style=filled, fillcolor="#d4edda", color="#28a745", fontsize=11, fontname="Fira Mono", fontcolor="#155724"];`
			);
		} else {
			// Non-terminal node — blue rounded box
			nodes.push(
				`  ${currentId} [label="${label}", shape=box, style="rounded,filled", fillcolor="#d0e4f5", color="#4a90d9", fontsize=12, fontname="Fira Mono", fontcolor="#1a4c7a"];`
			);
		}
...
export async function renderClosureAfnSvg(
	...
	try {
		...
		for (const alt of firstProd.children) {
			...
			startItems.push({
				parent: firstProd.parent,
				symbols,
				dot: 0,
				lookahead: first ? '$' : undefined
			});
		}
		...
		while (queue.length > 0) {
			...
			edges.push(
				`  S${currentId} -> S${advancedId} [label="${sym}", fontsize=10, fontname="Fira Mono", color="#495057"];`
			);
				...
				for (const prod of g.productions) {
					...
					for (const alt of prod.children) {
						...
						for (const la of lookaheads) {
							...
							edges.push(
								`  S${currentId} -> S${epsilonId} [label="ε", style=dashed, color="#adb5bd", fontcolor="#adb5bd", fontsize=10, fontname="Fira Mono"];`
							);
...
export async function renderAFNItemsSvg(
	...
	try {
		...
		const hasLookahead =
			states.some((s: LRState) => s.items?.some((i: LRItem) => i.lookahead)) ?? false;
		...
		for (const state of states) {
			...
			nodes.push(
				`  S${state.id} [label=${htmlLabel}, shape=rect, style="rounded,filled", fillcolor="#ffffff", color="${borderColor}", penwidth=1.5, margin="0.2,0.1"];`
			);
		}
		...
		for (const [key, target] of transitions) {
			...
			edges.push(
				`  S${from} -> S${target} [label="${sym}", fontname="Fira Mono", fontsize=10, ${style}];`
			);
		}
...
export async function renderGotoSvg(
	...
	try {
		...
		for (const stateId of targetStates) {
			nodes.push(
				`  "S${stateId}" [label="S${stateId}", shape=circle, style=filled, fillcolor="#d0e4f5", color="#4a90d9", fontsize=11, fontname="Fira Mono"];`
			);
		}
		...
		for (const [key, target] of transitions) {
			...
			const edgeStyle = isEpsilon
				...
				: 'color="#495057", fontcolor="#495057"';
			edges.push(
				`  "S${from}" -> "S${target}" [label="${sym}", fontname="Fira Mono", fontsize=10, ${edgeStyle}];`
			);
		}
